...
document.getElementById('save-character').addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.id) {
            alert(`Personagem salvo com sucesso! ID: ${data.id}`);
        } else {
...
document.getElementById("load-character").onclick = async () => {
    const id = prompt("Digite o ID do personagem:");
    ...
    try {
        ...
        alert(`Personagem carregado: ${character.name}`);
        // Aqui você pode preencher os dados do personagem nos campos do formulário.
    } catch (err) {
        console.error(err);
...
function getCharacterData() {
    return {
        ...
        sanityCurrent: document.getElementById("sanity-current").value,
        sanityMax: document.getElementById("sanity-max").value,
        manaBlocks: document.getElementById("mana-blocks").value,
        history: document.getElementById("char-history").value,
        skills: {},  // Adicione aqui suas skills quando implementado
        attacks: []   // Adicione aqui seus ataques quando implementado
    };
}
